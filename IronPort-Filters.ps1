...
quarantine_spoofed_protected_senders: if ((mail-from-dictionary-match("Protected_User_Names", 1)) OR (header-dictionary-match("Protected_User_Names","From", 1))) AND ((sendergroup != "WHITELIST") AND (sendergroup != "DLP") AND (sendergroup != "RELAYLIST") AND (sendergroup != "Authorized_888Holdings") AND (sendergroup != "WHITELIST_External") AND mail-from-dictionary-match("Safe_Display_Name_Impersonators", 1)) {
notify-copy ("$EnvelopeSender", "", "", "Recipient") ;                               
quarantine("Ext_888Holdings_Spoof");
                                         }
